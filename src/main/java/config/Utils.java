...
public class Utils extends BaseTest {
	...
	public static void stepStarted() {
		try {
			...
			step.log(Status.INFO,
					MarkupHelper.createLabel("Paso iniciado: " + value, ExtentColor.BLUE));
			System.out.println("Paso iniciado: " + value);
		} catch (Exception e) {
	...
	@Context(description = "Captura la pantalla y guarda la imagen en Base64 y la agrega como extra al reporte")
	public static void takeAdditionalScreenshot(String methodName, String folder) {
		...
		try {
			...
			Files.copy(f, image);
			byte[] imageBytes = readImageToBytes(image.getAbsolutePath());
			step.pass("Captura adicional", MediaEntityBuilder.createScreenCaptureFromBase64String(encodeBytesToBase64(imageBytes)).build());
	...
	@Context(description = "Obtiene el nombre de la variable de un elemento de forma relativa")
	public static String elementNameRelativeClass(String variable) {
		try {
			StackTraceElement[] stackTrace = Thread.currentThread().getStackTrace();
			String className = "";
			for (int st=0 ;st < stackTrace.length; st++ ) {
				if(stackTrace[st].getClassName().contains(".pageEvents.")) {
					 className = stackTrace[st].getClassName();
					 break;
				}
	...
	public static String getName(WebElement element, String xpathElement) {
		...
		try {
			...
			if (elementName.equalsIgnoreCase("") || elementName.isEmpty() || elementName.equalsIgnoreCase("null")|| elementName.contains(":")) {
	...
	public static void errorLOG(ITestResult result) {
		...
		if (exception != null) {
			...
			step.log(Status.FAIL, "No se ha completado el evento " + eventErrorMethodName + " a causa de: " + Utils.exceptionMessage);
			System.out.println("No se ha completado el evento " + eventErrorMethodName + " a causa de: " + Utils.exceptionMessage);
